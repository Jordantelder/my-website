...
function handout() {
  ...
  c.push(h3('Model 1'));
  c.push(p('Identify product risks that could adversely affect patients or users, then select a minimum of one element to evaluate requirements in each of the six QMS Areas, plus the four OAFRs as applicable, plus the general items: registration and listing, marketing authorizations, previous 483 and compliance issues, and any areas defined in the assignment.'));
  c.push(h3('Model 2'));
  c.push(p('Evaluate a prescribed minimum list of elements: Product and Process Changes; the design and development elements from inputs through transfer, including software validation; Management Review, Medical Device File and Planning of Product Realization; Analysis of Data, Control of Nonconforming Product, Complaint Handling, Feedback, Internal Audits, Corrective Action and Preventive Action; Validation of Processes, Control of Production and Service Provision, Identification and Traceability, and, for sterile product, Sterile Medical Devices and Validation of Processes for Sterilization and Sterile Barrier Systems; Outsourcing. That is 22 elements, or 23 for sterile product. Figure 2 then adds the same general items as Model 1, and the four OAFRs.'));
  c.push(callout(['**The OAFR split inside Model 2.** Model 2 has only two uses, and they do not carry the same OAFR scope. On a baseline surveillance inspection the four OAFRs are evaluated. On a PMA preapproval inspection they are not: each of the four OAFR sections (Part III.D.1 to 4) reads "Evaluate during all risk-based inspections except for PMA Preapproval inspections", and Part III.C directs the investigator, for a PMA preapproval inspection of a subject device not on the market in the United States, to "exclude elements marked OAFR in the model". Medical Device Tracking is narrower still: it is evaluated only where a tracking order was issued under 21 CFR 821. A crosswalk that presents "22 or 23 elements plus four OAFRs" as the whole of Model 2 is wrong for half of its uses.']));
  c.push(small('Neither list is a ceiling. Both figures carry the footnote that if the inspection reveals objectionable conditions, or the minimum requirements do not let the investigator assess something adequately, additional elements should be considered; and for most inspection types the Figure 3 model assignment applies "unless otherwise specified by the assignment". FDA has made the same point publicly: both models define minimum requirements and are intentionally flexible. Surveillance inspections are not conducted at sites actively enrolled in MDSAP, but MDSAP participants may be inspected under compliance follow-up, for-cause and Specific Product Risk Assignment assignments.'));
